...
(def crypto-hmac-sha-256 (new HMac hash/digest-sha-256))
(def length-hmac-sha-256 (. crypto-hmac-sha-256 getMacSize))

(with-test
  (defn hmac-sha-256 [ data crypto-key ]
    (let [ data-bytes    (byte-arrays/output data) 
           data-length   (count data)
           result-bytes  (byte-arrays/create length-hmac-sha-256) ]
      (. crypto-hmac-sha-256 reset)
      (. crypto-hmac-sha-256 init (new KeyParameter crypto-key))
      (. crypto-hmac-sha-256 update data-bytes 0 data-length)
      (. crypto-hmac-sha-256 doFinal result-bytes 0)
