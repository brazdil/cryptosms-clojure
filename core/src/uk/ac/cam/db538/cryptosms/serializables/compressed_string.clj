(ns uk.ac.cam.db538.cryptosms.serializables.compressed-string
  ...
  (:require [uk.ac.cam.db538.cryptosms.utils :as utils]
            [uk.ac.cam.db538.cryptosms.compressed-string :as compressed-string]
...
(with-test
  (defn compressed-string
    ...
    (uk.ac.cam.db538.cryptosms.serializables.common.Serializable.
      ; export
      (fn [data]
        (compressed-string/compressed-string (data key-name)))
      ; import
      (fn [^bytes xs args] 
        (if (< (count xs) 1)
          (throw (new IllegalArgumentException))
          {key-name (compressed-string/compressed-string xs)} ))
      ; length
      (fn [data] (count (compressed-string/compressed-string (data key-name)))) ))
...
(with-test
  (defn compressed-string-aligned
    ...
    (uk.ac.cam.db538.cryptosms.serializables.common.Serializable.
      ; export
      (fn [data]
        (compressed-string/compressed-string-aligned (data key-name)))
      ; import
      (fn [^bytes xs args] 
        (if (< (count xs) 16)
          (throw (new IllegalArgumentException))
          {key-name (compressed-string/compressed-string-aligned xs)} ))
      ; length
      (fn [data] (count (compressed-string/compressed-string-aligned (data key-name)))) ))
  ; tests just check that everything is integrated together correctly... string compression methods are tested separately
  (let [ data                   {:string "Hello world! I'm CryptoSMS"}
