...
(with-test
  ...
  (let [ item (uint/uint64 :id)
    ...
    (is (thrown? IllegalArgumentException (create -1 item)))
    (is (thrown? IllegalArgumentException ((:export (create 7 item)) data)))
    (is (= (count ((:export (create 128 item)) data)) 128))
    (is (= (subvec ((:export (create 128 item)) data) 0 8 ) result))
    (is (= ((:import (create 16 item)) result-aligned {}) data))
    (is (= ((:length (create 128 item)) data) 128)) ))
